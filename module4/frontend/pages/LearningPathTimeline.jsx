...
const LearningPathTimeline = () => {
    ...
    if (!path) return <div className="text-gray-500">Learning path not found.</div>;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
                ...
                <div className="border-l-4 border-[#F5EBE0] ml-4 pl-8 space-y-12">
                    {path.sequence && path.sequence.map((lesson, idx) => (
                        <div key={lesson.lessonId} className="relative group">
                            {/* Marker */}
                            ...
                            <div className={`bg-white p-6 rounded-xl border transition-all 
                                ...
                                {lesson.status === 'current' && (
                                    <button className="bg-[#8B5E3C] text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-[#6D4930] transition-colors w-full sm:w-auto">
                                        Continue Learning
                                    </button>
                                )}
                    ...
                    <ul className="space-y-3">
                        {reviews.map(item => (
                            <li key={item.lessonId} className="flex justify-between items-center bg-[#FAFAFA] p-3 rounded-lg border border-gray-50 hover:border-[#F5EBE0] transition-colors">
                                ...
                                <button className="text-xs text-[#8B5E3C] hover:underline font-bold">Review</button>
                            </li>
                        ))}
                    </ul>
                )}
