...
export default function ParentDashboardMain() {
    ...
    const handleLinkChild = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch('http://localhost:5000/api/parent/link-child', {
                method: 'POST',
                headers: {
    ...
    const viewChildProgress = async (child) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`http://localhost:5000/api/parent/child/${child._id}/progress`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const data = await response.json();
            setProgress(data);
